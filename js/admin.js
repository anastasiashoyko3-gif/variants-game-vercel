...
function renderAll(){renderPlayers();renderScore();renderAdminState();renderRevealPanel();renderNextHint()}
...
function renderScore(){
  ...
  $('scoreBoard').innerHTML=arr.length?arr.map((p,i)=>`<div class="playerRow rank${i+1}"><div class="avatarLine">${avatarHtml(p)}<b>${i===0?'🥇':i===1?'🥈':i===2?'🥉':i+1+'.'} ${escapeHtml(p.name)}</b></div><b>${p.score||0}</b></div>`).join(''):'<p class="muted">Поки немає балів.</p>';
}
...
function renderQuestionEditor(){
  ...
  $('questionsForm').innerHTML=Array.from({length:TOTAL_QUESTIONS},(_,i)=>{const q=byOrder.get(i)||{},r=roundNo(i);return `<div class="qedit"><h3>Питання ${i+1} · Раунд ${r} ${r===3?'<small>можна фото</small>':''}</h3><textarea id="q_${i}" placeholder="Текст питання">${escapeHtml(q.text||'')}</textarea><input id="c_${i}" placeholder="Правильна відповідь" value="${escapeHtml(q.correct_answer||'')}"/><input id="f_${i}" placeholder="Фейк ведучої" value="${escapeHtml(q.fake_answer||'')}"/><input id="url_${i}" type="hidden" value="${escapeHtml(q.photo_url||'')}"/>${q.photo_url?`<img class="thumb" src="${escapeHtml(q.photo_url)}" alt="Фото">`:''}${r===3?`<label class="fileLabel">Фото до питання<input id="file_${i}" type="file" accept="image/*"></label>`:''}</div>`}).join('');
}
...
async function doAction(action){
  ...
  if(action==='next_question'){const next=Number(currentGame.current_q||0)+1;if(next>=questions.length)await updateGame({phase:'finished',status:'finished',finished_at:new Date().toISOString()});else await updateGame({current_q:next,phase:'question_preview',scoreboard_visible:0,answer_deadline:null,vote_deadline:null})}
  await loadData();
}
