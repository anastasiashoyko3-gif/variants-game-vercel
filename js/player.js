...
async function refreshState(){
  if(!game||loading)return;loading=true;
  try{
...
function render(){
  ...
  if(currentQuestion&&game.phase==='preview'){const opts=safeJson(currentQuestion.options_json);html=questionHtml()+`<h2>Варіанти</h2><p class="muted">Поки тільки читаємо.</p>${opts.map((o,i)=>`<button class="option" disabled>${i+1}. ${escapeHtml(o.text)}</button>`).join('')}`}
  if(currentQuestion&&game.phase==='voting'){
    const left=deadlineLeft(game.vote_deadline), opts=safeJson(currentQuestion.options_json), myVote=votes.find(v=>Number(v.player_id)===Number(player.id)); html=questionHtml()+timerHtml(left)+'<h2>Голосування</h2>';
  ...
  if(game.phase==='finished')html='<h2>Гра завершена</h2>'+scoreHtml();
  stateBox.innerHTML=html;startTimer();
}
function questionHtml(){return currentQuestion?`<div class="pill">Раунд ${currentQuestion.round_no} · питання ${Number(game.current_q)+1}</div><div class="question">${escapeHtml(currentQuestion.text)}</div>${currentQuestion.photo_url?`<img class="photo" src="${escapeHtml(currentQuestion.photo_url)}" alt="Фото">`:''}`:''}
