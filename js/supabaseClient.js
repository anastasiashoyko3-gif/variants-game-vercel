...
export function avatarHtml(person, size = '') {
  ...
  if (avatar.startsWith('http')) {
    return `<span class="${cls}" style="background-image:url('${escapeHtml(avatar)}')"></span>`;
  }

  return `<span class="${cls}">${escapeHtml(avatar || ((person && person.name) || '?')[0])}</span>`;
}

export function hostAvatarHtml(game, size = '') {
  const avatar = (game && game.host_avatar) || '👑';
  return avatarHtml({ name: 'Ведуча', avatar }, size);
}
