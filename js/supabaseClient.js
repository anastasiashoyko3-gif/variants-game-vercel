...
export function avatarHtml(person,size=''){const avatar=(person&&person.avatar)||'';const cls=`avatar ${size}`.trim();if(/^https?:\/\//i.test(avatar))return `<span class="${cls}"><img src="${escapeHtml(avatar)}" alt=""></span>`;return `<span class="${cls}">${escapeHtml(avatar||((person&&person.name)||'?')[0])}</span>`;}
