...
let game=null, questions=[], currentQuestion=null, players=[], answers=[], votes=[], channel=null, timerInterval=null, pollInterval=null, loading=false;
...
function finalScreenHtml(){
  ...
  return `<div class="winnerBox"><div class="winnerCup">🏆</div><h2>Гра завершена</h2><p class="muted">Переможець гри</p><div class="avatarLine winnerLine">${avatarHtml(winner,'big')}<b>${escapeHtml(winner.name)}</b></div><div class="winnerPoints">${winner.score||0} балів</div></div>${scoreHtml()}`;
}
